...
<x-layout :title='$title'>
    <x-card class="mb-4 fadeInTop relative">
        <img src="{{ asset('img/everest.png') }}" alt="everest img"
            class="object-cover absolute right-0 h-full w-full lg:w-6/12 top-0 opacity-20 bg-gradient-to-r from-transparent">
        ...
        <div class="z-20">
            <h1 class="text-4xl font-bold text-lime-600 leading-6">{{ $user->name }}
                @if (auth()->user()->role == 'admin')
                    <a href="/users/{{ $user->id }}/edit" title="Edit User">
                        <i class="fa-duotone fa-pen-to-square lagani-theme"></i>
                    </a><br>
                @endif
                <span class="text-xl text-gray-400 font-normal capitalize">{{ $user->role }}</span>
            </h1>
